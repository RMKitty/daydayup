...
- 1) 协议: 只是 **约定**, 如果你想要 **具备某个能力** 时, 你 **必须做** 的事情

- 2) Foundation 提供的 2个 让一个类, **具备对象拷贝能力** 的协议
  - NSCopying 协议
  - NSMutableCopy 协议
...
- 4) **Foundation** 大多数系统类
  ...
  - 所以才可以对 Foundation 类对象, 直接发送 copy、mutableCopy 消息

- 5) **自定义类型** 对象拷贝
  - 首先, 需要自己实现 **NSCopying** 和 **NSMutableCopy** 这2个协议, **具备** 拷贝功能
  - 再次
    - 并不是, 实现了 **NSCopying** 协议, 就具备 **浅拷贝** 能力
    - 也不是, 实现了 **NSMutableCopy** 协议, 就具备 **深拷贝** 能力

- 6) 这2个 copy 协议, 只是说 **具备拷贝** 这个能力, 至于是 **怎么拷贝(浅、深)** 是取决于 **你自己的实现** 方式
...
### 1. NSObject 提供, 两种 对象拷贝 api

- 1) `-[NSObject copy]`
- 2) `-[NSObject mutableCopy]`

### 2. 如果你要发送 `-[NSObject copy]` 消息
...
### 3. 如果你要发送 `-[NSObject mutableCopy]` 消息
...
### 4. 实现哪一种协议, 取决于你的类, 是否同时存在 ==可变== 和 ==不可变== 两个版本
